...
class MenuImages :
	...
	def __init__(self, parent, options) :
		# Creation d'un objet manu en images gif
		self.boutons = []
		self.width = 20
		self.height = 20
		self.hor = 'horizontal'
		...
		#--------------------------------------------------------
		# Creation de la barre d'outils (c'est un simple cadre) :
		#--------------------------------------------------------
		##toolbar = Frame(self.mainframe, bd =1,  bg ='white')
		##toolbar.pack(expand =YES, fill =X)
		# Nombre de boutons a construire :
		"""
...
class oracleHelper :
	...
	def connexion (chaine) :
		...
		try :
			...
			messages = Pmw.MessageDialog(self.page0, title="Connexion base",) 
                        #message_text="Impossible de se connecter a la base")
			#print "Impossible de se connecter a la base "
			return
		return connection
	...
	def testConnect(self):
		...
		while i < 3 :
			...
			try :
				...
				self.parent.afficheInfos( "Erreur de connexion", "connexion a echouer" + chaine )
...
class clssTestConnect(threading.Thread):
	def __init__(self, parent, listChaine=[]):
		...
		#Declaration du texte deroulant  
		...
		parent.infoBase( "Je suis tout seul a tester ok" ) 
		self.afficheInfos( "Je suis tout seul a tester ok")
	...
	def afficheInfos(self, text):
		...
		#on insète le texte 'texte' dans le widget 'texte deroulant' nomme 'st'
	...
	def testConnectMain(self):
		...
		while i < 3 :
			...
			try :
				...
				self.afficheInfos( "Erreur de connexion connexion a echouer" + chaine )
	...
	####################
	def scanDB(self) :
		#scan a partir du fichier tnsnames.ora
		#
		...
		j = 0 
		dicoDB = {}
		##re_nombre = re.compile(r"(\d+)") # on exprime, on compile l’expression regulière
		## 4 resultat = re_nombre.search(une_chaine) #renvoie l’objet None en cas d’echec
	...
	def testConnect(self):
		...
		try :
			...
			## Message Alert 
			tkMessageBox.showerror("Erreur de connexion", "connexion a echouer")
			self.connection = None 
...
class BarreProgression(threading.Thread) :  
	...
	def iconise(self, after=2000) :
		#iconise la fenetre barre progression
...
class MessageDialogBoxAs :
	...
	def __init__(self) :
		...
		"""
		#
		comment = Label(pframe, text="Vous voulez enregistrer ce parametrage ?").pack(fill=BOTH, expand=1, padx=20, pady=20)
...
class clss_spool_transactions (Listbox):
	...
	def afficheOptionsTransaction(self, id_curselection) :
		...
		#dans libelle transaction je recupere l'ID 
	...
	def updateTransactionFileDataXml(self, elem_transaction, new_value) :
		...
		for transaction in les_transaction :
			...
			if elem_transaction == x_nom.firstChild.data :
				...
				"""
				#print "transactionent trouve %s ancien = %s" % (elem_transaction , new_value)
				# = "xxxxxxxx"
				break
...
class ScrooledList(Frame) :
	...
	def __init__(self, options,  parent=None, label=tkinter.Label, height=2, width=25, command=None) :
		...
		Frame.__init__(self, parent) # attache a une Frame
