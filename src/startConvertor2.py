...
class ClssStartConvertor(ClssConvertor) :
	...
	def concat_videos(self) :
		...
		for infile in self.src_filenames :
			...
			if infile.endswith(INPUT_EXT):
				...
				cmd = 'ffmpeg -i ' + infile + ' -c copy -vbsf h264_mp4toannexb -s 800x450 ' + outfile
				os.system(cmd)
				log.info(cmd)
	...
	def concat_fichiers_audio(self):
		...
		"ffmpeg -i son.wav -i video.yuv out.mpg",
		"""
		log = logging.getLogger('concat_fichiers_audio')
		INPUT_EXT = "mp3"
		files = ""
		
		for infile in self.src_filenames :
				fichier_in 	= infile.decode("utf8", 'ignore')
				print "morceaux en entree : %s endwith=%s" % (infile, infile.endswith(INPUT_EXT))
	
				if infile.endswith(INPUT_EXT):
					#infile = os.path.basename(infile)
					infile = infile.replace(' ', '\ ')
					if infile == self.src_filenames[-1]:
						files = files +   infile 
					else :
						files = files +   infile + '|'
		
		cmd = "avconv -i concat:'" + files + "' -c copy   out/out.mp3"
		print "command=%s" % (cmd)
