...
from commun_helpers import subprocessLaunchWithoutConsole, searchConfigUserFromFileXml, createElemntXml, creerRepertoire, deleteConfigUserFromFileXml
# module general
import threading
import Tkinter
import Pmw
...
class ClssStartConvertor(ClssConvertor) :
	...
	def interfaceStart(self) :
		...
		bt4 = self.buttonBoxAdmin.add('Paramètrage',  	cursor='hand2',width=100, height=40,
		...
		self.tipinfo1.bind(bt2, 'Paramètrage du lanceur')
		#
		#-----------------------------
	...
	def affPage2(self):
		...
		'fusionner un fichier audio et un fichier vidéo pour créer une vidéo avec une piste sonore' : "ffmpeg -i son.wav -i video.yuv out.mpg",
		...
		# Pour connaître le numéro de votre écran $ env | grep DISPLAY
		"Faire une capture vidéo de votre écran et FFMPEG" : "ffmpeg -b 1000k -f x11grab -s 1024x768 -r 30 -i :0.0 capture.mpg",
		...
		#Il s’agit d’extraire une image de la vidéo et d’en faire une miniature pour donner un aperçu de la vidéo.
		#-vcodec : spécifie le codec de sortie (ici mjpeg pour obtenir une image au format jpg)
		#-vframes : nombre d’images à capturer
		#-ss : position de l’image dans le temps
		"Extraire une image d’une vidéo" : "ffmpeg -i %s -vcodec mjpeg -vframes 1 -an -s 200x200 -ss 30 %s" ,
		...
		self.list_cmd_ffmpeg_video = {
			...
			"Encoder une vidéo pour l'iPhone ou l'iPod Touch" : "ffmpeg -i %s -acodec aac -ab 128kb -vcodec mpeg4 -b 1200kb -mbd 2 -flags +4mv+trell -cmp 2 -subcmp 2 -s 320x180  %s",
			...
			# Pivoter la video de 90 degré a droite
			...
			"Assembler des videos en un seul fichier vidéo" : "ffmpeg -y -i concat:%s|%s -absf aac_adtstoasc -c copy -f mov %s"
		}
	...
	def affPage3(self) :
		...
		self.list_cmd_ffmpeg_audio = {
			"Extraire le son en qualté superieur"	: "ffmpeg -i %s -vn -ac 2 -ar 44100 -ab 320k -f %s -y '%s' ",
			# wav to mp3
			'convertir un fichier wav en mp3 à 22050 Hz' : "ffmpeg -i %s -ar 22050 -f %s %s -y ",
	...
	def afficheCmdVideo(self, key_command) :
		...
		ffcommand = self.list_cmd_ffmpeg_video.values()[key_command]
	...
	def afficheCmdAudio(self, key_command) :
		...
		# contruire la commande 
		ffcommand = self.list_cmd_ffmpeg_audio.values()[key_command]
		print ffcommand
	...
	def convert_M2TS_to_avi(self, fichier_source=""):
		...
		#fichier_source = re.escape(basename_video_source) # escape les cataere accentué
		...
		if os.path.isfile (fichier_source) :
			...
			if mswindows :
				...
				"""
				fichier_source 	= json.dumps(fichier_source, ensure_ascii=False)  # escape les quotes # escape les cataere accentué
	...
	def infoFormat(self, fichier) :
		...
		# recherche des proprité de la video ou du mp3
		#---------------------------------------------
	...
	def parametrages(self) :
		...
		self.page1 = self.notebook.add('Paramètres')
	...
	def convertVideo(self, fichier_source, format_video, format_audio, codec_video, r_width, r_height) :
		...
		#print fic_out
		#---------------------------------------------
		# 1- Convertir la video ou musique récuperer
		#---------------------------------------------
	...
	def ajouterVideo(self) :
		...
		#------------------------------------
		lesfichiers  = askopenfilename(**options)
...
"Format TS" : "Un fichier TS est un fichier d'un DVD vidéo. Un DVD vidéo peut contenir des données destinées à des platines de salon ainsi que des données",
